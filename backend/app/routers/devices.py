...
def list_devices(
    ...
    total, items = crud.list_devices(db, search=search, status_id=status_id, type_id=type_id, skip=skip, limit=limit)
    return {"total": total, "items": items}


@router.post("/", response_model=schemas.DeviceRead, status_code=status.HTTP_201_CREATED)
def create_device(device: schemas.DeviceCreate, db: Session = Depends(get_db), user=Depends(get_user)):
...
def delete_device(device_id: int, db: Session = Depends(get_db), user=Depends(get_user)):
    device = crud.get_device(db, device_id)
    if not device:
        raise HTTPException(status_code=404, detail="Device not found")
